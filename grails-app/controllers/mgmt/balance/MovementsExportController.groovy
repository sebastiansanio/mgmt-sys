...
class MovementsExportController {
	...
	private static final String QUERY = """	SELECT movement.date_created AS movement_date_created,
			 coalesce(movement_item.amount*movement_item.multiplier,0) AS movement_item_amount,movement_item.date AS movement_item_date,
		     movement_item.date_created AS movement_item_date_created,movement_item.description AS movement_item_description,
		     coalesce(movement_item.iibb*movement_item.multiplier,0) AS movement_item_iibb,coalesce(movement_item.iva*movement_item.multiplier,0) AS movement_item_iva,
		     coalesce(movement_item.total*movement_item.multiplier,0) AS movement_item_total, movement.number AS movement_number,
