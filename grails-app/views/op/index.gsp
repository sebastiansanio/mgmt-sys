	...
	<table class="table table-bordered margin-top-medium">
		<thead>
			<tr>
				...
				<g:sortableColumn params="${params}" property="checked" title="${message(code: 'movement.checked.label')}" />
		...
		<g:each in="${movementInstanceList}" status="i" var="movementInstance">
			<tr class="${movementInstance.checked ? 'checked-movement' : ''}">
				...
				<td><g:link action="edit" id="${movementInstance.id}"><span class="glyphicon glyphicon glyphicon-pencil"></span></g:link></td>
				...
				<td><g:link controller="report" action="downloadReport" id="${mgmt.reports.Report.findByCode("op").id}" params="${[movement_id:movementInstance.id]}"><span class="glyphicon glyphicon-download-alt"></span></g:link></td>
